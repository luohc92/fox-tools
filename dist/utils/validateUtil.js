...
export const validateUtil = {
    /** 是否为空 */
    isNullOrEmpty: (val, strict = false) => {
    ...
    /** 是否为中文 */
    isChinese: (val, minLength, maxLength) => {
    ...
    /** 是否为纯英文 */
    isEnglish: (val, minLength = 0, maxLength = 0) => {
        const len = val.length;
        if (len < minLength)
            return false;
        if (maxLength !== 0 && len > maxLength)
    ...
    /** 是否为英文和数字 */
    isAlphanumeric: (val, minLength = 0, maxLength = 0) => {
        const len = val.length;
        if (len < minLength)
            return false;
        if (maxLength !== 0 && len > maxLength)
    ...
    isStrongPassword: val => /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{6,}$/.test(val),
    /** 是否为合法 URL，必须带 http:// 或 https:// */
    isURL: val => /^(https?:\/\/)([\w-]+\.)+[a-z]{2,}(:\d{1,5})?(\/\S*)?$/i.test(val),
    /** 是否为 IPv4 地址 */
    isIPv4: val => /^(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)){3}$/.test(val),
    /** 是否为 Base64 字符串 */
    isBase64: val => /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(val),
    /** 是否为十六进制颜色代码（#fff / #ffffff） */
    isHexColor: val => /^#(?:[0-9a-fA-F]{3}){1,2}$/.test(val),
    /** 是否为 UUID 格式 */
    isUUID: val => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(val),
    /** 是否为合法日期 */
    isDate: (val) => {
    ...
    isTime: (val) => /^([01]\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(val),
    ...
    /** 是否为 PDF 文件 */
    isPdfFile: (val) => /\.pdf$/i.test(val),
    /** 是否为 Excel 文件（含 WPS 表格） */
    isExcelFile: (val) => /\.(xls|xlsx|et)$/i.test(val),
    /** 是否为 Word 文件（含 WPS 文字） */
    isWordFile: (val) => /\.(doc|docx|wps)$/i.test(val),
    /** 是否为 PowerPoint 文件（含 WPS 演示） */
    isPptFile: (val) => /\.(ppt|pptx|dps)$/i.test(val),
};
